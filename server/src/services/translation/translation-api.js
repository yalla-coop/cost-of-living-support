...
const translate = async ({ source, target, json, id }) => {
  ...
  if (AWSTranslateJSONInst && AWSTranslateJSONInst.translateJSON) {
    console.log(
      'object :>> ',
      AWSTranslateJSONInst.translateJSON.TranslatedText,
    );
    ...
    try {
      ...
      throw new Error('translation api: try statement error');
    } catch (error) {
      throw new Error('translation api error: ', error);
    }
  } else {
    throw new Error(
      'translation api error: no AWS translation service initiated',
    );
  }
};
